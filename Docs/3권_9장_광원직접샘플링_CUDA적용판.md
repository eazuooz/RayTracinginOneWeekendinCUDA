...
> 원서의 흐름(광원의 PDF → 하드코딩 광원 샘플링 → 단방향 광원)을 따라가되 설명은 요약·재구성했고, 코드는 전부 우리 GPU 코드다. 실제로 빌드·렌더해서 노이즈와 시간을 측정했다.
...
광원 위에서 점을 **균일하게** 뽑으면 면적 기준 밀도는 넓이 A에 대해 $1/A$다. 그런데 우리가 적분하는 공간은 **방향(입체각)** 이다. 광원의 작은 조각 $dA$가 단위 구에 투영되면

$$ d\omega = \frac{dA\cdot\cos\theta}{\operatorname{distance}^2(p,q)} $$

가 된다($\theta$는 광원 법선과 시선 사이 각). 같은 조각을 뽑을 확률은 어느 공간에서 재든 같아야 하므로 $p(\omega)\,d\omega = p_q(q)\,dA$ 이고, $p_q(q) = 1/A$ 를 넣으면

$$ p(\omega) = \frac{\operatorname{distance}^2(p,q)}{\cos\theta \cdot A} $$

멀수록(거리² 커짐) 밀도가 커지고, 비스듬히 볼수록(cos 작아짐) 커진다 — 둘 다 "그 방향으로 보이는 광원이 작아진다"는 뜻이니 직관과 맞는다.
