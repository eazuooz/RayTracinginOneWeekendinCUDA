...
> 원서의 흐름(목표 → 코넬 박스 기준 이미지 → 균일 PDF 실험 → 반구 균일 산란)을 따라가되 설명은 요약·재구성했고, 코드는 전부 우리 GPU 코드다. 실제로 빌드·실행해 이미지와 수치를 확인했다.
...
- **산란 함수(재질)를 잘못 고르면** 다른 답에 수렴한다. 몬테카를로에서 가장 찾기 어려운 버그는 "그럴듯한 그림이 나오는 버그"다 — 5장의 cos³ Lambertian, 위의 편향된 균일 PDF가 모두 그랬다.
