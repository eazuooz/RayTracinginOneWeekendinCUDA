...
3장에서 우리는 균일 난수에서 출발해 점점 복잡한 난수 생성 방법을 쌓아 올렸고, 결국 **PDF**라는 직관과 **임의의 분포를 따르는 난수를 만드는 방법**에 도달했다. 그 장의 도구(PDF, $f/p$의 평균, 역함수 샘플링)는 **차원이 늘어도 그대로 통한다.** 2차원, 3차원 공간에서 점을 뽑고 그 선택에 PDF로 가중치를 주면 된다. 레이트레이싱에서 가장 중요한 경우는 **무작위 방향**을 만드는 일이다.
...
피적분 함수는 $f(\theta,\phi) = \cos^2\theta$ 이고, θ는 z축과 이루는 각이다.

여기서 좌표를 바꾸면 코드가 훨씬 단순해진다. 단위 벡터 $d$ 를 z축에 **스칼라 사영**하면

$$ d_z = \lVert d \rVert \cos\theta = 1\cdot\cos\theta $$

이므로 $\cos\theta$ 를 통째로 $d_z$ 로 바꿔 쓸 수 있다.

$$ f(\theta, \phi) = \cos^2\theta = d_z^{\,2} $$

삼각함수가 하나도 필요 없다 — 방향 벡터의 z성분을 제곱하면 끝이다. 몬테카를로 추정은 균일 방향 $d$ 를 뽑아 $f(d)/p(d) = 4\pi\,d_z^2$ 를 평균내는 것이다.
...
앞으로 나오는 "방향에 대한 적분"은 전부 **입체각에 대한 적분**($d\omega$)이다. 2차원 각도가 머리에 잘 안 들어온다면, 원서 저자가 권하는 대로 "그 방향들이 지나가는 단위 구 위의 넓이"를 떠올리면 된다. **입체각 $\omega$ 와 단위 구 위의 투영 넓이 $A$ 는 같은 것이다.**
