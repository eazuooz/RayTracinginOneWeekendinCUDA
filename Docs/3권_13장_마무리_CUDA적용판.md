...
원서는 이 책의 목적이 "물리 기반 렌더러의 **샘플링 방식**을 정리하면서 i의 점 하나, t의 가로줄 하나까지 빠짐없이 짚는 것"이었다고 말한다. 이제 이 세부 사항들을 들고 여러 갈래 길을 탐험할 수 있다는 것이다. 관심사에 따라 갈 만한 길을 이렇게 짚어 준다.
...
- **엄밀한 물리 기반** — RGB를 스펙트럼으로 바꾸기. 원서 저자는 **레이마다 무작위 파장 하나**를 들고 다니고 프로그램의 RGB 대부분을 float 하나로 바꾸는 방식을 특히 좋아한다고 적는다. 비효율적으로 들리지만 실제로는 그렇지 않다.
- **어느 길로 가든** — **광택(glossy) BRDF 모델**을 하나쯤은 추가해 보라는 조언. 고를 수 있는 모델이 여럿이고 각각 장점이 다르다.
