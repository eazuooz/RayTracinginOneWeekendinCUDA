...
몬테카를로 알고리즘의 교과서적인 예가 **π 추정**이다. π를 추정하는 방법은 여럿인데, 고전으로 꼽히는 것이 **뷔퐁의 바늘(Buffon's needle) 문제**다 — 폭이 같은 나무판을 나란히 깐 바닥에 바늘을 무작위로 떨어뜨렸을 때, 바늘이 두 판에 걸칠 확률을 묻는 문제다. 여기서는 그 아이디어에서 착안한 변형을 쓴다.
